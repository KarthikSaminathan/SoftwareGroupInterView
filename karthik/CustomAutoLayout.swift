...
extension UIView {
    ...
    func updateLocalConstraint(anchor: Any, constrints: [NSLayoutConstraint], constant: CGFloat) {
        if let layoutAnchor = anchor as? NSLayoutAnchor<AnyObject> {
            for constr in constraints {
                if layoutAnchor == constr.firstAnchor || layoutAnchor == constr.secondAnchor {
                    constr.firstAnchor.constraint(equalTo: anchor as! NSLayoutAnchor<AnyObject>, constant: constant).isActive = true
//                    var test = constr.firstAnchor.constraint(equalTo: constr.firstAnchor, constant: constant)
//                    test.isActive = true
                    self.layoutIfNeeded()
                    print("coming inside \(constr.firstAnchor)")
                    break
                }
            }
        } else {
